...
const marcaTareaRealizada = function(idTarea) {
   const posicion = arregloTareas.findIndex((e) => e.id == idTarea);
...
const dibujaLista = function(){
    ...
    for (const tarea of arregloTareas) {
       ...
       <tr>
          <td> ${tarea.id} </td>
          <td>${tarea.nombre}</td>
          <td><input type="checkbox" ${statusCheck} onclick="marcaTareaRealizada(${tarea.id})"><button class="btn btn-danger" onclick="eliminarTarea(${tarea.id});">Eliminar</button></td>
       </tr>`;
    }
    htmlElementosLista += '</tbody>' 

    listaTareas.innerHTML = htmlElementosLista;
    spanTotalTareas.innerHTML = arregloTareas.length;
